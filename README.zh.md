...
| | |
|---|---|
| **验证每一样东西，并告诉你怎么验的** | 一个岗位只有今天从雇主自己的页面抓到，才算还开着。链接能打开不等于你有资格申请，所以它先读资格条款。搜索返回空的时候，它会用同样的方法去搜一个确知存在的东西再决定信不信，因为"空"是关于搜索本身的事实。 |
| **自动扫描新岗位，但有边界** | 你自己定跑几轮、每轮几个。它会报告找到了什么，**也会报告哪些渠道没打开**，而且绝不为了凑数往里塞。六个真的，好过十个里有四个是猜的。 |
| **把"读不到"和"已关闭"分开** | 每个岗位都带一个标签说明是怎么核实的：确认开着、只在聚合站见过、被挡住、确认已下架。页面打不开是关于你的连接的事实，不是关于这个岗位的。把这两件事混为一谈，正是一个自动搜索开始骗你的方式。 |
| **任何事情发生之后复盘** | 面试、被拒、收到回复，或者沉默久到已经说明问题。当天做，趁你还记得。偶尔一次复盘会长出一条规则，这里每条规则都是这么来的。 |
| **规则跟着申请一起累积** | 目前十五条，每条都是先犯了错才写下来的。绝不自动投递。绝不编造数字、职称或截止日期。投入之前先读资格条款，因为它总在广告最底下，而截断的阅读永远读不到那里。 |
| **发出去之前先审一遍** | 单独一道关，六件事会直接拦下：追溯不到来源的说法、今天没重新核过的数字、把雇主名字换成竞争对手之后还读得通的信。写的人看不见自己写的东西，所以这一关必须是分开的。 |
| **记录你的文风和偏好** | 你实际怎么写字、哪些开头你绝不会用、哪些说法是你自己的、哪类岗位你想要哪类不要。**不是自动猜的，是你纠正的时候写下来的**，所以它一直是准的。 |
| **把所有东西汇成一张表** | 从你的记录文件生成，不手工维护，所以不会跑偏。它专门凸显两件最容易被忘掉的事：材料做完了却从没发出去的，和有名有姓却从来没联系过的人。 |
| **告诉你今天该做什么** | 每次都从记录里重新推导的一份短清单，绝不沿用上一次：一周内截止且还没发的、做完了却没定发送日期的、有路径却从没用过的。"今天没有什么需要你" 是一个有效的回答。 |
| **让你的求职留在私有** | 两个仓库，不是一个。发布前六道检查：你的标识符、顺带被点名的其他人、邮箱地址、私有路径、匿名化的残留。同样的检查在 CI 里也跑，**而且查历史不只查文件**，因为一个从文件里删掉但留在历史里的名字，仍然是公开的。 |
...
标识符清单故意不放在仓库里，所以 CI 从 repository secret 里读。**设置一次，否则 CI 会失败**，这是故意的：跑不起来的检查不等于通过的检查，而一个没有意义的绿勾比一个红叉更糟。
...
的**，没有它就没有这个仓库。那个项目的核心想法，也就是求职应该跑在你自己的 AI 编程命令行里而不是别人的
SaaS 里，就是这里的全部前提。
